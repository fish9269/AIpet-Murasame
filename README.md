...
| 层 | 说明 |
|---|---|
| **动机层** `tool/desire.py` | 三个需求随时间自己长：**无聊**（独处/屏幕久不变）、**想说话**（久没聊）、**精力**（陪着你消耗）。攒够了就产生一个念头，交给模型自己决定怎么做 —— 想放首歌就用【音乐】标记、想看你忙什么就【看屏幕】、想聊就找你说话、太累就安静陪着。每个念头有冷却，不会反复念同一样。 |
...
陪伴天数、动机层、活跃度档位、四层记忆+夜间整理、任务经验、自主分档、一键自检、游戏模式。

**还没做的（按价值排）**：

1. **记忆升级**：现在是 `pets/<角色>/memory/learned.json`（归纳 + 日记）。
   参考 AgentPet 的"四层记忆"和 Miru 的"槽位 + 日记 + 可读 Markdown"：
   把记忆分成 感官（当前屏幕/会话）、工作（正在做的事）、情节（互动历史）、语义（提炼出的偏好与事实），
   并按主题检索（关键词 + 时间衰减），夜里做一次"整理"（合并重复、丢弃过时）。
2. **开口时机（注意力引擎）**：Miru 的 AttentionEngine 会算"值不值得开口"。
   现在她是"定时看屏幕 + 有事就说"，可以升级成打分制（屏幕变化量、多久没说话、主人是否在打字）。
3. **情绪状态机**：情绪现在只用于当轮立绘，可以加一个随时间衰减的心情值，影响语气与主动性。
4. **关系度**：互动频率、摸摸头、被夸/被凶 → 记录一个数值，影响称呼与亲疏。
5. **任务规划与经验**：现在的任务循环是"一步一规划"，可以加失败重规划，并把"某个软件怎么开"
   这类成功路径记成经验，下次直接用。
6. **安全边界**：给自主行动分档（哪些能自己做、哪些必须先问），所有操作已有日志可查。
