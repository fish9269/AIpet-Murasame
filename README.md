...
| fee | 含义 | 没会员时能听吗 |
|---|---|---|
| **0** | 完全免费 | ✅ **能整首放**（实测切成 fee=0 的歌后，"正在试听"提示就消失了） |
| 8 | 低音质免费 | ❌ 实测仍是 30 秒试听 |
...
**先说清楚能力边界**（实测数据）：一轮循环 = 抓屏 22ms + 视觉认画面 5~10 秒 +
规划 2~4 秒 + 执行 → **一轮 8~15 秒**。所以：

| 能玩 | 玩不了 |
|---|---|
| 回合制、战棋、卡牌、解谜、文字冒险、挂机/放置、需要反复刷材料的 | 动作类、竞技类（火影、无畏契约、FPS、吃鸡）——帧率差两个数量级 |

两种玩法：
- **看着画面玩**（需要判断的）：你说「你帮我打一局」，她会问/用你教的操作方式：
  `【游戏】开始 纸嫁衣：目标 通关第一关 ；操作 空格 推进、数字键 选项`
  之后每轮：抓屏 → 认画面 → 决定 1~3 个动作 → 执行 → 继续
- **固定循环**（刷材料，不花模型额度、很快）：
  `【游戏】连按 J 30 次 间隔 0.8 秒` / `【游戏】连点 800 250 20 次`
- **盯屏触发**（最快、最省：不花模型额度）：盯住屏幕上一小块，
  画面一变就立刻动手 —— `【游戏】盯着 800 400 100 50 变化就 按键 space`
  （实测每秒检查 4 次、约 0.25 秒就反应；比走模型快几十倍，
  适合「某个提示一出现就按一下」的挂机/简单游戏）
- `【游戏】停` / `【游戏】会玩哪些`

**安全**：玩的时候挂 **F12 急停**；**主人一开口说话她立刻停手让位**；默认最多 10 分钟 / 60 轮；
动作走键鼠同一套保护（坐标校验、写日志，并关掉"重复抑制"以便连按连点）；
操作方式会记进 `memory/games.json`（下次不用再教）。开关在菜单「电脑与学习」→「允许她玩游戏」。

**它属于自主控制的一部分**（不是单独一个功能）：她的**动机层**里有一个「自己玩会儿」的念头——
主人开着「自主操作」、又离开电脑超过 5 分钟，她就会挑一个玩过的游戏自己玩几分钟；
你一回来（动一下鼠标键盘）或者一开口，她立刻停下来陪你。
