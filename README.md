...
## 游戏功能第六轮修（2026-09-24）：六个问题 + 又参考了开源做法

用户一次报了六件事，逐条修：

| 问题 | 原因 | 修法 |
|---|---|---|
| 「显示手上正在玩别的」 | 上一次的"在玩"状态**卡住没清**（循环异常退出时残留） | `start()` 先自检：**心跳超过 120 秒**或窗口已消失 → 判定为残留状态，清掉重来；循环每轮更新心跳 |
| 游戏没了还显示"正在玩游戏" | 结束时的空状态被忽略了 | 收到空状态 → **把对话框还给她上一句话**；收尾同时清掉点击框/后台点击目标 |
| **点的位置不对** | 她看到的是**游戏窗口**的画面，坐标却按**整屏**算 ✗ | 新增「点击框」：玩某个窗口时把窗口矩形设进去，方位词（中央/中下部…）**按窗口换算**；提示词也要求游戏里只用方位词、别猜像素 |
| 对话没说完被顶掉 | 整轮回复在**句子之间**有停顿，那会儿 `_talking` 是 False → 状态文字挤进来 ✗ | 新增 `_reply_active`：`on_reply` 开始到结束**整轮都算忙**，状态只能等她说完 |
| 直接抢鼠标 | 用的是真鼠标（SendInput） | 新增**后台点击** `_post_click()`（PostMessage + ScreenToClient，**不动真鼠标、不抢光标**）；真鼠标兜底也会**把光标还原**；若后台点击没被游戏接受（画面无变化）→ **自动改用真鼠标重点一次** |
| 菜单还有「允许她玩游戏」 | 我说过它属于自主控制，却还留着独立开关 | 菜单项**删掉**，`enabled()` 恒为可用：主人开口随时能玩；她自己想玩由「自主操作」开关决定 |

**参考的开源做法**（GUI Agent 领域通用套路）：
干净的窗口画面（OmniParser：只喂目标窗口）、`act → verify`（UI-TARS 那类：每步执行后
校验画面变化，没变就换做法）、区域/方位词定位（SeeAct：先给"大致区域"再细化，比让模型
猜绝对像素稳）、以及"状态要能自愈"（别让上一次的残留状态挡住下一次）。

## 「恶魔轮盘没正常游玩」→ 参考开源 GUI Agent 改了四点（2026-09-24 第六轮）

用户反馈。日志里看到的症状：视觉把它认成"RPG 的标题界面"（其实是**浴室场景 + AFRAID 标志**），
然后对着 (960,650) **连点 5 次**，对话队列还被系统消息堆到 6 条。

按开源 GUI Agent（OmniParser / SeeAct / UI-TARS 这类）的通用做法改：

| 改法 | 说明 |
|---|---|
| **只截游戏窗口** | 以前抓整屏 → 桌宠自己、任务栏都混进去，模型常把桌宠当画面内容。新增 `capture_window_qimage()`（PrintWindow + 退回按窗口矩形裁剪）。**实测效果立竿见影**：描述变成了游戏真实画面（"浴室…写着 AFRAID 的标志…下方中间有个继续按钮"）✓ |
| **动作后校验（act→verify）** | 每步执行后比一次画面指纹（`_verify_changed()`）：变化 < 0.02 就认为"点空了" → 下一轮**强制完整看一次**、并且**不再快进重复它**，还会在给模型的"上一轮做了什么"里注明"画面一点没变，可能没生效" |
| **别猜像素，用方位词** | 规划器提示词里加：点不准就用 中央／左上／右上／左下／右下／顶部／底部／中下部／中间（桌宠按屏幕比例换算），「点击 960 650」这种瞎猜容易点空 |
| **对话队列去重** | 干活时的系统消息（"你做完了一次操作"）会堆队列 → 她一直在"回排队的消息"。现在同类的只留最新一条（主人的话永远保留） |
...
## 「让她玩游戏没反应」（2026-09-24 第四轮）

用户反馈。日志里看到的其实是**她说不出话**，不是没执行：

```
[游戏] 没找到「ATRI」的窗口，而且前台也不是游戏 → 不开玩
[桌宠] 🎮 游戏：我没找到「ATRI」的窗口——你先把它打开、让它在最前面，我再上手。
[桌宠] ⏳ 她还在说话/思考，这条先排队（队列 1）… 卡在「她还在说话」超过 30 秒 → 释放对话锁
```

两个问题：

1. **它把"找不到 ATRI"判断对了，但没法告诉你** —— 那句结果要等模型转述，
   而她当时还忙着 → 排队 → 卡住 → 看起来就是"点了没反应" ✗。
   **修法**：结果**立刻直接显示**在对话框（不等模型），模型那句随后再补上 ✓。
2. **名字对不上时不肯接手** —— 以前回退用"前台窗口"，可主人往往是**先点桌宠说话**
   → 前台就是桌宠自己（被过滤掉）→ 回退永远为空 → 明明开着游戏也说找不到 ✗。
   **修法**：回退改成"**Z 序最上面那个真窗口**"（EnumWindows 的第一个，就是主人正看着的），
   并且播报里说明拿的是哪个：「我没找到叫「ATRI」的窗口，看到你开着《Buckshot Roulette》，
   就先玩这个了」。

## 游戏提速 / 关掉就停 / 点歌文案（2026-09-24 第三轮）

### 1. 玩得太慢 → 加"快进模式"

一轮完整流程 = 抓屏 + 视觉认画面（几秒~十几秒）+ 规划（几秒）≈ **15~25 秒**，
视觉小说这种"点一下就继续"的根本不值得每轮都重新看一眼。

现在：**上一轮的动作如果可以安全重复**（点击/双击/右键/按键/等待），
就直接重复它 —— 连着 **4 次快进（每次间隔 0.6 秒）** 再完整看一次画面
（等于替主人连点「继续」）。带打字（输入文字）或组合键的**不会**快进，
必须每轮重新看一眼再决定（免得乱打字/乱切窗口）。
顺带把游戏看图的档位从 640/100 降到 **560/70**（快一点）。

### 2. 关掉游戏还显示在游玩 → 每轮按名字复查 + 收尾清状态

- 以前只查"当初记下的窗口句柄还活着吗"，如果当初是**靠前台窗口兜底**认出来的
  （句柄可能记的是别的窗口）→ 关掉游戏也检测不到 ✗。
- 现在 `find_game_window()` 会告诉我们"是不是按名字认出来的"，是的话**每轮再按名字找一遍**，
  名字找不到了就认为游戏关掉 → 停手 + 如实说 ✓。
- 循环结束时**清掉状态文案**（不再挂着"正在玩…"）。

### 3. 点歌文案："正在帮你点歌" → "正在点歌"

用户反馈：让她放**她自己**喜欢的歌时，对话框写「正在**帮你**点歌」很矛盾 ✗ → 改成中性文案。
...
## 「游戏卡着不动」+ 说话与文案（2026-09-24 再修一轮）

用户反馈四件事，一条条来：

### 1. 游戏还是卡着不动 → 真因：**被她自己的内部提示掐停了**

日志实锤：

```
[游戏] 开始玩「ATRI」→ 窗口《ATRI -My Dear Moments-》   ← 游戏正常接上了
[游戏] ⛔ 停止（主人开口说话了）                        ← 立刻被停掉 ✗
[游戏] 第 1 轮画面：…文字冒险游戏…「下方中间有个开始按钮」
[游戏] 第 1 轮决定：【键鼠】点击 960 650                ← 只做到第 1 轮
```

她接下游戏后，桌宠会发一条**内部提示**（「（系统提示：你刚接下了陪主人玩游戏…说一句）」）
去让她开口 —— 这条提示用的是 `role="user"` ✗，于是被"主人一开口就停手让位"的逻辑
当成**你真开口了** ✗ → 游戏第一轮就被掐停 ✓。

**修法**：`start_thread` 里区分真主人与内部提示 —— 凡是 `（系统提示：` 开头的都不算主人开口
（不停游戏、也不刷新"主人刚说过话"的时间戳 ✓ 顺带让她平时不那么"装哑巴"）。

### 2. 「放我喜欢的音乐」回复还带"我" → 是歧义

「我喜欢的音乐」是**网易云里主人收藏的那个歌单的名字**，但在她嘴里"我"是她自己 ✗
→ 听起来像在放她自己的歌。提示词里写清楚了，并要求回话时说明是**主人的**歌单
（「你收藏的那张歌单给你放上了」）。

### 3. 状态文案去掉「第 N 轮 / 第 N 步」

`正在玩 ATRI……第 3 轮` → **`正在玩 ATRI……`**；
`正在操作电脑……第 5 步` → **`正在操作电脑……`**（轮/步数只留在日志里，不再显示）。

### 4. 玩游戏 / 操控电脑 / 做任务时要时不时说话

新增 **say 通道**：两个循环的规划器里加了「每 2~3 轮顺口说一句」的要求，
她会多写一行 `【说】<很短的一句人话>`（如「点了一下继续，剧情在走」）；
循环把这一句抽出来 → 桌宠 `_say_progress()` → **走正常对话管线**（有台词、有立绘、有语音）
让她说出来 ✓ 不再是闷头干活。

## 「游戏都没打开，她就显示在游玩」（2026-09-24 修）

用户反馈。原因很直接：`game.start()` **从来不检查游戏窗口在不在** ——
模型一句 `【游戏】开始 ATRI …` 就把状态切成"在玩"，然后**对着桌面一轮轮空点**，
还一直说"我正在玩" ✗（游戏没开、她当然点不动）。

修法（`tool/game.py`）：
- 新增 `find_game_window(name)`：在**可见窗口**里按名字模糊匹配
  （"ATRI" 能对上 "ATRI -My Dear Moments-"）；名字对不上就用**前台窗口**
  （主人说"玩这个游戏"指的就是眼前这个）；都不行 → **不开玩**，
  老实说「我没找到《X》的窗口——你先把它打开、让它在最前面，我再上手」。
  - 窗口标题里常夹**零宽空格**（"Microsoft​ Edge"），归一化时一并去掉，
    否则匹配不上（实测踩到）；
  - 系统壳窗口（NeXuS/NxDock/Twinkle/截图工具/桌宠自己那个 Qt 窗口…）都排除在外。
- `start()` 里记下窗口句柄与标题，播报也带上：「我来玩《X》（窗口是《…》）」。
- 游戏循环**每轮检查窗口还在不在**：`_window_alive()` 返回 False 就立刻停手并说
  「《X》好像关掉了，我就不玩了。想再玩就把它打开喊我。」✓（用户明确要求的行为）

实测：
- 没开游戏时喊她玩 → 拒绝 + **`running()` 仍是 False** ✓（不再假装在玩）；
- 窗口匹配：`Edge` / `Microsoft Edge` / `DeepSeek` 都能对上 ✓；
- 假窗口句柄 → `_window_alive` = False ✓（会停手如实说）。

## 「让她玩视觉小说游戏为什么不动」（2026-09-24 查到三个原因）

用户反馈。日志里逐条对照，是三个原因叠在一起：

### 1. 她压根没进"玩游戏"那条路（最主要）

日志里她全程只发 `【键鼠】移动 …`，**一次都没写过 `【游戏】开始 …`** → 走的是**键鼠任务循环**
→ 那个循环每轮反过来问她"你要我玩什么" → 她只能一直回"你告诉我点哪儿/选哪条线" → **不动** ✓
（历史里连着五六轮都是"你要我玩也行，先说说你想让我选哪条线"）。

改法（`tool/game.py` 提示词）：
- 明确写清**视觉小说/文字冒险完全能玩**（不用手速）：点画面中央/按空格就是「继续」，
  遇选项点选项；
- 明确要求**直接写完整的开始标记、不要问主人选哪条线、不要等他确认**，并给了范例：
  `【游戏】开始 ATRI：目标 帮他推进剧情 ；操作 点击屏幕中下部继续、按空格也能继续、遇选项点选项`；
- 规划器里也补了一句：这类游戏每轮就写一行「点击 960 650 / 按键 space」，**别停下来问**；
- 另外修坐标写法：「移动 中间 620」这种**数字和方位词混着写**执行不了（实测踩到）——
  提示词里加了规则，并补了「中下部/下半屏/中间」等方位词能估出位置。

### 2. 本地视觉服务早就没了，"看屏幕"一直失败

实测调用时**连接被拒**（服务进程不在了：手动起的会随终端结束、也可能被清理），
而原来只是"回落云端"——云端没配好，她就**彻底看不见**，表现就是"不动/说看不到画面"。

改法（`tool/chat.py`）：新增 `_ensure_local_vision()` —— 本地视觉连不上时
**照着启动器那套把服务拉起来**（找带显卡加速的运行时 → 起服务 → 等健康检查，最多 20 秒）
再重试一次。实测："服务不在 → 已把它拉起来 → 服务已就绪" ✓ 然后真的描述了画面 ✓。

### 3. 她看到的"画面"是她自己

日志里她的原话："**屏幕上就是我嘛**——那个桌宠窗口里坐着的动漫角色" ✗
（桌宠窗口压在最上面，视觉模型自然先描述它）。

改法：看游戏画面 / 任务看屏幕时，提示词里加一句
「画面里那个 AI 桌宠窗口是**你自己**，请忽略它，重点描述后面的游戏/窗口」。

自检：28 模块 + 84 处接线 + 19 项全过。
