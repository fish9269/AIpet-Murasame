...
_ANCHORS = (("左上", 0.06, 0.06), ("右上", 0.94, 0.06),
            # 视觉小说/文字冒险常写「点中下部继续」——这几个说法也要能估出位置
            ("中下部", 0.5, 0.80), ("下半屏", 0.5, 0.78), ("画面中央", 0.5, 0.5),
            ("正中", 0.5, 0.5), ("中间", 0.5, 0.5),
...
PROMPT_RULES = (
    ...
    "★ **数字和方位词不能混着写**：「移动 中间 620」「点击 底部 400」这种执行不了（实测踩到过）——"
    "要么写两个数字「点击 960 620」，要么只写方位词「点击 中下部」；"
    "「中下部」「下半屏中间」这类描述性方位也可以（会按屏幕比例估位置）。\n"
