...
_KW_TYPE = {
    ...
    "按键": "key", "按下": "key", "按": "key", "key": "key", "press": "key",
...
PROMPT_RULES = (
    ...
    "★ 说了就要做到：你说了「帮你点掉」就必须真的输出「点击」指令。只写「移动」等于什么都没做——"
    "那是把鼠标挪过去而已，不许拿它敷衍，更不许嘴上说做了其实没做（做不到就直说做不到）。\n"
    "★ 位置估不准时的正确做法：先输出「【键鼠】移动 800 250」把鼠标挪过去，再输出「【看屏幕】」"
    "看清鼠标底下是什么，确认后再「【键鼠】点击 800 250」。只想点大致方位也可以写"
    "「【键鼠】点击 中央／左上／右上／左下／右下」。\n"
    "★ 一条指令只做一个动作；要做两件事就写两行（先点哪里、再输入什么）。"
    "指令那一行不会被念出来，也不用向主人朗读；不需要动手时不要输出这类指令。"
)
