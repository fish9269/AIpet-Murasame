...
class LongTextVoice:
    ...
    def __init__(self, ref_audio=None, ref_text=None):
        ...
        # 默认：长语音也用"日常说话那套参考"，避免同一只桌宠两套音色来回跳
        # （想要长语音包自己的音色：config.json 里 gsv_unified_ref = false）
        try:
            _u = self._cfg("gsv_unified_ref", True)
        except Exception:
            _u = True
        if str(_u).lower() not in ("false", "0", "no") and not ref_audio:
    ...
    def say(self, msg: str, save_path: str = None, callback=None):
        ...
        try:
            import hashlib
            try:
                _eff = self._effective_ref(msg.strip())[0] if self.backend == "gpt_sovits" else self.ref_audio
            except Exception:
                _eff = self.ref_audio
            ...
            try:
                ...
                pass
            _ck = hashlib.md5(("%s|%s|%s|%.2f|%s|%s|%s" % (msg.strip(), _eff, self.backend,
                                                          self._speed, self.gsv_steps, _fp2,
                                                          self._model_tag)).encode("utf-8")).hexdigest()[:16]
    ...
    def _effective_ref(self, text: str):
        ...
        prompt_lang = "ja" if _kana(self.ref_text) else "zh"
        text_lang = "ja" if _kana(text) else prompt_lang
        # ⚠ 长语音只用长语音包（用短语音的样例会让音色变味，用户明确要求不要混用）；
        #   文本语言按参考音频的语言走，保证"用谁的声音"始终是同一套音色。
        return self.ref_audio, self.ref_text, prompt_lang, prompt_lang
